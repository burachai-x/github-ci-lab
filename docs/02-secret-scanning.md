...
## ขอบเขตของการตรวจประวัติ: `--all` ทำให้ `main` แดงจากความผิดของ branch อื่น

ตอนแรกเราสั่งให้รอบที่รันบน `main` ตรวจด้วย `--log-opts="--all"` ซึ่งหมายถึง
**ทุก ref ใน repo** ผลคือ `main` ขึ้นแดงทันทีที่มี branch ตัวอย่างซึ่งจงใจใส่ค่าลับไว้
ทั้งที่ตัว `main` เองสะอาด

```
27 commits scanned.
WRN leaks found: 6      <- มาจาก branch demo/01 ที่ยังไม่ได้ merge
```

check ที่แดงด้วยเรื่องที่คนดู `main` แก้ไม่ได้ คือจุดเริ่มต้นของการที่ทีมเลิกสนใจ CI
เราจึงแยกขอบเขตตามเหตุที่รัน

| เหตุที่รัน            | ขอบเขต                                       | บล็อกไหม            |
| --------------------- | -------------------------------------------- | ------------------- |
| `pull_request`        | `base..HEAD` เฉพาะ commit ที่ PR เพิ่มเข้ามา | ✅ บล็อก            |
| `push` เข้า `main`    | `HEAD` ประวัติของ `main` เท่านั้น            | ✅ บล็อก            |
| `schedule` รายสัปดาห์ | `--all` ทุก branch ใน repo                   | ❌ รายงานอย่างเดียว |

แบบนี้ทั้ง "ไม่บล็อกคนที่ไม่ได้ทำผิด" และ "ยังมีคนรู้ว่ามีของค้างอยู่ใน branch ไหน"
